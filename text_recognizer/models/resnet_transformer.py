...
class ResnetTransformer(nn.Module):
    def __init__(self, data_config: Dict[str, Any], args: argparse.Namespace = None) -> None:
        ...
        idx_to_char = {val: ind for ind, val in enumerate(data_config["char_to_idx"])}
        self.start_token = idx_to_char["<S>"]
        self.end_token = idx_to_char["<E>"]
        self.padding_token = idx_to_char["<P>"]
        self.max_output_length = data_config["output_dim"][0]
    ...
    def encode(self, x: torch.Tensor) -> torch.Tensor:
        """
        Parameters: (B, H, W) image

        Returns: torch.Tensor(Sx, B, E) logits
        """
        _B, C, _H, _W = x.shape
        if C == 1:
            x = x.repeat(1, 3, 1, 1)
    ...
    def decode(self, x, y):
        ...
        Returns: torch.Tensor(Sy, B, C) logits
        """
    ...
    def forward(self, x: torch.Tensor, y: torch.Tensor) -> torch.Tensor:
        ...
        Returns: torch.Tensor(B, C, Sy) logits
        """
    ...
    def predict(self, x: torch.Tensor) -> torch.Tensor:
        ...
        Returns: torch.Tensor(B, Sy) with elements in [0, C-1] where C is num_classes
        """
        B = x.shape[0]
        ...
        for Sy in range(1, S):
            ...
            output_tokens[:, Sy : Sy + 1] = output[-1:].view(-1, 1) # Set the last output token
