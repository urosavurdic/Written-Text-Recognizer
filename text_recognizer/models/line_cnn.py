    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        Input:
            x (B, C, H, W): imput image:
                ...
                W - Width
        Returns:
            torch.Tensor of dimensions (B, C, H, W)
        """
        c = self.conv(x)
        b = self.bn(c)
        r = self.relu(b)
...
class LineCNN(nn.Module):
    ...
    def __init__(self, data_config: Dict[Any, Any], args: argparse.Namespace = None) -> None:
        ...
        self.convs = nn.Sequential(
            ...
            ConvBlock(conv_dim, conv_dim),
            ConvBlock(conv_dim, 2*conv_dim, stride=2),
            ConvBlock(2*conv_dim, 2*conv_dim),
            ConvBlock(2*conv_dim, 4*conv_dim, stride=2),
            ConvBlock(4*conv_dim, 4*conv_dim),
