...
DATA_DIRNAME =  BaseDataModule.data_directory_path() / "processed" / "emnist_lines2"
...
class EMNISTLines2(BaseDataModule):
    ...
    Similarly implemented as EMNIST Lines
    Adds augmentations (brightness jitter, affine rotation/scale/slant, Gaussian noise). Much closer to real-world handwriting variability.
    Difference in some attributes and ways of describing some
    """
    def __init__(self, args: argparse.Namespace = None):
        ...
        max_width = int(self.emnist.dim[2] * (self.max_length + 1) * (1 - self.min_overlap)) + IMAGE_X_PADDING
        ...
        self.output_dim = (MAX_OUTPUT_LENGTH, 1)
    ...
    def data_filename(self):
        return (DATA_DIRNAME / f"ml_{self.max_length}_o{self.min_overlap:f}_{self.max_overlap:f}_ntr{self.num_train}_ntv{self.num_val}_nte{self.num_test}.h5")
    ...
    def _generate_data(self, split: str) -> None:
        ...
        with h5py.File(self.data_filename, "a") as f:
            ...
            y = convert_strings_to_labels(y, emnist.idx_to_char, length =MAX_OUTPUT_LENGTH)
...
def create_dataset_of_images(N, samples_by_char, sentence_generator, min_overlap, max_overlap, dim):
    images = torch.zeros((N, IMAGE_HEIGHT, dim[2]))
    labels = []
    for n in range(N):
        label = sentence_generator.generate()
        crop = construct_image_from_string(label, samples_by_char, min_overlap, max_overlap, dim[-1])
        height = crop.shape[0]
...
def get_transform(augment=False):
    ...
    return transforms.Compose([
        transforms.ColorJitter(brightness=(0.5, 1)), transforms.RandomAffine(degrees=3, translate=(0, 0.05), scale=(0.4, 1.1), shear=(-40, 50), resample=Image.BILINEAR, fillcolor=0), transforms.ToTensor(),
        ])
