...
PROCESSED_DATA_DIRNAME = BaseDataModule.data_directory_path() / "processed" / "iam_paragraphs"
...
class IAMParagraphs(BaseDataModule):
    """
    Synthetic paragraphs dataset based on the IAM Handwriting Database.
    """
    ...
    def setup(self, stage: str = None) -> None:
        ...
        def _load_dataset(split: str, augment: bool) -> BaseDataset:
            """
            Loads the dataset for the given split ('train_val' or 'test') and applies data augmentation if specified.
            """
...
def validate_input_and_output_dimensions(
    ...
    """
    Validates the input and output dimensions for the IAMParagraphs dataset.
    """
    ...
    assert output_dims is not None and output_dims[0] >= properties["label_length"]["max"] + 2  # +2 for start and end tokens

def get_dataset_properties() -> dict:
    """
    Reads the dataset properties from the _properties.json file
    Returns:
        A dictionary with min and max values for: label_length, num_lines, crop_shape, and aspect_ratio.
    """
    with open(PROCESSED_DATA_DIRNAME / "_properties.json", "r") as f:
        properties = json.load(f)

    def _get_property_values(key: str) -> list:
        return [_[key] for _ in properties.values()] # returns list of values for given key
...
def get_paragraph_crops_and_labels(iam: IAM, split: str) -> Tuple[Dict[str, Image.Image], Dict[str, str]]:
    """
    Takes an IAM object and returns paragraph crops and their corresponding labels for the specified split.
    For each form in the IAM dataset, it crops the paragraph region based on the bounding boxes of the lines present in that form. 
    It then combines the text from all lines in the paragraph into a single label, separating lines with a special NEW_LINE_TOKEN.
    Returns:
        crops (Dict[str, Image.Image]): Dictionary mapping paragraph IDs to their image crops.
        labels (Dict[str, str]): Dictionary mapping paragraph IDs to their text labels.
    """
    crops = {}
    labels = {}
...
def save_crops_and_labels(crops: Dict[str, Image.Image], labels: Dict[str, str], split: str) -> None:
    """"
    Saves the paragraph crops and labels to the processed data directory.
    """
...
def _labels_filename(split: str) -> Path:
    """
    Returns the path to the labels JSON file for the specified split.
    """
    return PROCESSED_DATA_DIRNAME / split / "_labels.json"

def _crop_filename(id_: str, split: str) -> Path:
    """
    Returns the path to the image crop file for the specified paragraph ID and split.
    """
...
def load_processed_crops_and_labels(split: str) -> Tuple[Sequence[Image.Image], Sequence[str]]:
    ...
    Returns:
        ordered_crops (Sequence[Image.Image]): List of image crops ordered by paragraph ID.
        ordered_labels (Sequence[str]): List of text labels ordered by paragraph ID.
    This ordering ensures that the i-th crop corresponds to the i-th label.
    """
...
def get_transform(image_shape: Tuple[int, int], augment: bool) -> transforms.Compose:
    """
    If augment is True:
        returns a composition of data augmentation transforms including random cropping, color jitter, and random affine transformations
    If augment is False:
        returns a composition that only includes center cropping and conversion to tensor
    """
    if augment:
        transforms_list = [
