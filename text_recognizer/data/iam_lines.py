...
PROCESSED_DATA_DIRNAME = BaseDataModule.data_directory_path() / "processed" / "iam_lines"
TRAIN_FRAC = 0.8
IMAGE_HEIGHT = 56
...
class IAMLines(BaseDataModule):
    ...
    def setup(self, stage: str = None) -> None:
        ...
        if stage == "fit" or stage is None:
            ...
            assert self.output_dim[0] >= max([len(_) for _ in labels_trainval]) + 2  # Add 2 for start/end tokens.

            y_trainval = convert_strings_to_labels(labels_trainval, self.idx_to_char, length=self.output_dim[0])
        ...
        if stage is None:
            self._verify_output_dim(labels_trainval=labels_trainval, labels_test=labels_test)

    def _verify_output_dim(self, labels_trainval, labels_test):
        max_label_length = max([len(label) for label in labels_trainval + labels_test]) + 2
        output_dim = (max_label_length, 1)
        if output_dim != self.output_dim:
            raise RuntimeError(output_dim, self.output_dim)
